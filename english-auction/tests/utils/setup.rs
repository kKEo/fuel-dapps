use fuels::{
  ...
  prelude::{
      abigen, launch_custom_provider_and_get_wallets, AssetConfig, Bech32ContractId, Contract, ContractId, LoadConfiguration, StorageConfiguration, TxPolicies, WalletUnlocked, WalletsConfig, BASE_ASSET_ID,
  },
...
abigen!(Contract(
    name = "EnglishAuction",
     abi = "out/debug/english-auction-abi.json"
),);

const AUCTION_CONTRACT_BINARY_PATH: &str = "out/debug/english-auction.bin";
const AUCTION_CONTRACT_STORAGE_PATH: &str = "out/debug/english-auction-storage_slots.json";
